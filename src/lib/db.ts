...
// New functions for property and related data retrieval
export const getPropertiesBySeller = async (sellerId: string) => {
  const result = await query('SELECT * FROM properties WHERE seller_id = $1', [sellerId]);
  return result.rows;
};

export const getPropertyTypes = async () => {
  const result = await query('SELECT * FROM property_types');
  return result.rows;
};

export const getPropertyStatuses = async () => {
  const result = await query('SELECT * FROM property_statuses');
  return result.rows;
};
...
export const getPropertyOperationStatuses = async () => {
  const result = await query('SELECT * FROM property_operation_statuses');
  return result.rows;
};
